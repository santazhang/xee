...
static NSInteger XeePDFSortPages(id first, id second, void *context)
{
	...
	if (!firstpage && !secondpage)
		...
		return [firstpage compare:secondpage];
}
	...
	@try {
		...
		if ([parser needsPassword]) {
			for (;;) {
				...
				if (!password)
					@throw @"Cancelled password request";
		...
		while (image = [enumerator nextObject]) {
			...
			NSString *name;
			if (page)
				name = [NSString stringWithFormat:@"Page %@, object %d", page, [ref number]];
			else
				name = [NSString stringWithFormat:@"Object %d", [ref number]];

			NSString *imgname = [[image dictionary] objectForKey:@"Name"];
			if (imgname)
				name = [NSString stringWithFormat:@"%@ (%@)", imgname, name];
...
- (XeeImage *)produceImage
{
	...
	if ([object isJPEG]) {
		...
		if (subhandle)
			newimage = [[[XeeJPEGImage alloc] initWithHandle:subhandle] autorelease];
	} else if ([object isJPEG2000]) {
		CSHandle *subhandle = [object JPEGHandle];
		if (subhandle)
			newimage = [[[XeeImageIOImage alloc] initWithHandle:subhandle] autorelease];
		...
		newimage = [[[XeeBitmapRawImage alloc] initWithHandle:subhandle
														width:[dict intValueForKey:@"Width" default:0]
													   height:[dict intValueForKey:@"Height" default:0]]
			autorelease];

		if (decode)
			[(XeeBitmapRawImage *)newimage setZeroPoint:[[decode objectAtIndex:0] floatValue] onePoint:[[decode objectAtIndex:1] floatValue]];
		else
			[(XeeBitmapRawImage *)newimage setZeroPoint:0 onePoint:1];
		...
		if (subhandle)
			newimage = [[[XeeRawImage alloc] initWithHandle:subhandle
													  width:[dict intValueForKey:@"Width" default:0]
													 height:[dict intValueForKey:@"Height" default:0]
													  depth:bpc
												colourSpace:XeeGreyRawColourSpace
													  flags:XeeNoAlphaRawFlag] autorelease];
		...
		if ([subcolourspace isEqual:@"DeviceRGB"] || [subcolourspace isEqual:@"CalRGB"]) {
			...
			if (palettedata) {
				...
				if (subhandle)
					newimage = [[[XeeIndexedRawImage alloc] initWithHandle:subhandle
																	 width:subwidth
																	height:subheight
																	 depth:bpc
