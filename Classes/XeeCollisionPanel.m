	 ...
	 didEndSelector:(SEL)selector
{
	...
	[titlefield setStringValue:[NSString stringWithFormat:NSLocalizedString(@"The file \"%@\" already exists.",
														  ...
														  [[destpath lastPathComponent] stringByMappingColonToSlash]]];
	...
	do {
		newpath = [destdir stringByAppendingPathComponent:[[NSString stringWithFormat:@"%@-%d", destname, n++] stringByAppendingPathExtension:destext]];
...
{
	[self endWithReturnCode:2
					   path:[[destinationpath stringByDeletingLastPathComponent]
