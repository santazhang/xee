...
+ (BOOL)canOpenFile:(NSString *)name firstBlock:(NSData *)block attributes:(NSDictionary *)attributes;
{
	...
	if (len >= 9 && !memcmp(head, "/* XPM */", 9))
		return YES;
	if (len >= 6 && !memcmp(head, "! XPM2", 6))
		return YES;
...
- (void)load
{
	...
	if ([firstline hasPrefix:@"/* XPM */"])
		version = 3;
	else if ([firstline hasPrefix:@"! XPM2"])
		version = 2;
	else
		version = 1;
	...
	if (version != 1) {
		...
		for (int i = 0; i < numcolours; i++) {
			...
			if (matches) {
				...
				if (hex != [XADRegex null])
					col = [self parseHexColour:hex];
				else {
				...
				if (col)
					[cols setObject:col forKey:code];
			}
		}
	} else {
		int numcolours = 0;
		...
		for (;;) {
			...
			if ((matches = [line substringsCapturedByPattern:@"^#define[ \t]+[^ \t]+_(format|width|height|ncolors|chars_per_pixel)[ \t]+([0-9]+)"])) {
				...
				if ([name isEqual:@"width"])
					...
					height = value;
				if ([name isEqual:@"ncolors"])
					numcolours = value;
				else if ([name isEqual:@"chars_per_pixel"])
					numchars = value;
			} else if ((matches = [line substringsCapturedByPattern:@"^static[ \t]+char[ \t]+\\*[ \t]*[^ \t]+_colors[ \t]*\\[[ \t]*\\][ \t]*=[ \t]*{"]))
				break;
		}

		if (!width || !height || !numcolours || !numchars)
			return;
		...
		for (int i = 0; i < numcolours; i++) {
			...
			if (matches)
				col = [self parseHexColour:[matches objectAtIndex:1]];
			else {
...
{
	if (version == 2)
		return [[self handle] readLineWithEncoding:NSASCIIStringEncoding];
	else
