...
- (id)initWithImage:(XeeImage *)img
{
	if (self = [super initWithImage:img]) {
		...
		if (untransformable && cropping)
			[self setControl:[XeeSLGroup groupWithControls:untransformable, cropping, nil]];
		else if (untransformable)
			[self setControl:untransformable];
		else if (cropping)
			[self setControl:cropping];
	}
	return self;
}
...
- (BOOL)save:(NSString *)filename
{
	...
	if (untransformable && [untransformable value] == 0)
		flags |= XeeRetainUntransformableBlocksFlag;
	if (cropping && [cropping value] == 1)
		flags |= XeeTrimCroppingFlag;
