...
- (void)destinationPanelEnd:(NSOpenPanel *)panel
				 returnCode:(NSInteger)res
				contextInfo:(void *)info;
- (void)transferToDestination:(NSInteger)index mode:(XeeDrawerMode)mode;
- (void)attemptToTransferCurrentImageTo:(NSString *)destination
								   mode:(XeeDrawerMode)mode;
- (void)collisionPanelEnd:(XeeCollisionPanel *)panel
			   returnCode:(NSInteger)res
					 path:(NSString *)destination
					 mode:(XeeDrawerMode)mode;
