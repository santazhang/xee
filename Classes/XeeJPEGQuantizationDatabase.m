...
+ (XeeJPEGQuantizationDatabase *)defaultDatabase
{
	...
	if (!database)
		database = [[XeeJPEGQuantizationDatabase alloc]
		...
		if (fh) {
			...
			while (fgets(buf, sizeof(buf), fh)) {
				...
				*colon = 0;
				...
				if (!array)
					[dict setObject:array = [NSMutableArray array] forKey:table];
				...
				if (stop) {
					*stop = 0;
					...
					if (!array)
						[dict setObject:array = [NSMutableArray array] forKey:shorttable];
					[array addObject:label];
				}
...
- (NSArray *)producersForTables:(struct jpeg_decompress_struct *)cinfo
{
	...
	if (num_quant_tables == 3 && cinfo->jpeg_color_space == JCS_YCbCr) {
		...
		for (int i = 0; i < 64 && dupe_table; i++)
			if (cinfo->quant_tbl_ptrs[1]->quantval[i] != cinfo->quant_tbl_ptrs[2]->quantval[i])
				dupe_table = NO;
