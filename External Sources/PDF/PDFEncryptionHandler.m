	...
	if (self = [super init]) {
		...
		if (version == 1 || version == 2) {
			...
			while (key = [enumerator nextObject]) {
				...
				if ([cfm isEqual:@"V2"])
					[algorithms setObject:[[[PDFRC4Algorithm alloc] initWithLength:length handler:self] autorelease] forKey:key];
				else if ([cfm isEqual:@"AESV2"])
					[algorithms setObject:[[[PDFAESAlgorithm alloc] initWithLength:length handler:self] autorelease] forKey:key];
				else
